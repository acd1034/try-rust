...
impl<'a> GenFun<'a> {
  fn new(module: &'a mut Module) -> GenFun<'a> {
    GenFun {
      module,
      builder: None,
      scope: Scope::new(),
  ...
  fn set_insert_function(&mut self, fun_id: FunctionId) {
    self.builder = Some(Builder::new(self.module.functions_get_mut(fun_id)));
  }
  ...
  fn builder_mut(&mut self) -> &mut Builder {
    &mut self.builder.unwrap()
  }

  // ----- gen_fun -----

  fn gen_fun(mut self, fun: TopLevel) -> Expected<FunctionId> {
    match fun {
      ...
      TopLevel::FunDef(ret_ty, name, param_tys, param_names, body) => {
        ...
        let fun_id = self.gen_fun_decl(ret_ty, name, param_tys.clone())?;
        self.set_insert_function(fun_id);
        ...
        // Create entry block
        let bb = self.builder_mut().append_basic_block();
        self.builder_mut().position_at_end(bb);
  ...
  fn gen_fun_decl(
    ...
    if let Some(fun_id) = self.module.get_function(&name) {
      let previous_ret_ty = &self.module.funs[fun_id].ret_ty;
      let previous_param_tys = &self.module.funs[fun_id].param_tys;
  ...
  fn create_entry_block_alloca(&mut self, _ty: Type, name: String) -> MemoryId {
    // Push mem_arena
    let mem_id = self.builder_mut().build_alloca();
    // Insert scope
  ...
  fn gen_if_else(
    ...
    todo!()
    // let current_block = self.builder_mut().get_insert_block();
    // let then_block = self
    //   .builder_mut()
    ...
    // // then:
    // self.builder_mut().position_at_end(then_block);
    // let has_terminator_in_then = self.gen_stmt(*then)?;
    // if !has_terminator_in_then {
    //   self.builder_mut().build_unconditional_branch(merge_block);
    // }

    // // else:
    // let has_terminator_in_else = if let Some(else_) = else_ {
    //   self.builder_mut().position_at_end(else_block);
    ...
    //   self.builder_mut().remove_basic_block(merge_block);
    //   Ok(true)
    // } else {
    //   self.builder_mut().position_at_end(merge_block);
    //   Ok(false)
    // }
  }
  ...
  fn gen_for(
    ...
    //   self.gen_expr(expr)?;
    // }
    // self.builder_mut().build_unconditional_branch(cond_block);

    // // cond:
    // self.builder_mut().position_at_end(cond_block);
    ...
    // } else {
    //   self.builder_mut().build_unconditional_branch(body_block);
    // }

    // // body:
    // self.builder_mut().position_at_end(body_block);
    // let has_terminator_in_body = self.gen_stmt(body)?;
    // if !has_terminator_in_body {
    //   self.builder_mut().build_unconditional_branch(inc_block);
    // }

    // // inc:
    // self.builder_mut().position_at_end(inc_block);
    ...
    // // end:
    // let has_no_branch_to_end = self.builder_mut().bb_arena[end_block].pred.is_empty();
    // if has_no_branch_to_end {
    //   self.builder_mut().remove_basic_block(end_block);
    // } else {
  ...
  fn gen_expr(&mut self, expr: AST) -> Expected<InstId> {
    match expr {
      AST::Eq(n, m) => {
        ...
        let v0 = self.builder_mut().build_eq(v1, v2);
        Ok(v0)
      }
      AST::Ne(n, m) => {
        let v1 = self.gen_expr(*n)?;
        let v2 = self.gen_expr(*m)?;
        let v0 = self.builder_mut().build_ne(v1, v2);
        Ok(v0)
      }
      AST::Lt(n, m) => {
        let v1 = self.gen_expr(*n)?;
        let v2 = self.gen_expr(*m)?;
        let v0 = self.builder_mut().build_lt(v1, v2);
        Ok(v0)
      }
      AST::Le(n, m) => {
        let v1 = self.gen_expr(*n)?;
        let v2 = self.gen_expr(*m)?;
        let v0 = self.builder_mut().build_le(v1, v2);
        Ok(v0)
      }
      AST::Add(n, m) => {
        let v1 = self.gen_expr(*n)?;
        let v2 = self.gen_expr(*m)?;
        let v0 = self.builder_mut().build_add(v1, v2);
        Ok(v0)
      }
      AST::Sub(n, m) => {
        let v1 = self.gen_expr(*n)?;
        let v2 = self.gen_expr(*m)?;
        let v0 = self.builder_mut().build_sub(v1, v2);
        Ok(v0)
      }
      AST::Mul(n, m) => {
        let v1 = self.gen_expr(*n)?;
        let v2 = self.gen_expr(*m)?;
        let v0 = self.builder_mut().build_mul(v1, v2);
        Ok(v0)
      }
      AST::Div(n, m) => {
        let v1 = self.gen_expr(*n)?;
        let v2 = self.gen_expr(*m)?;
        let v0 = self.builder_mut().build_div(v1, v2);
        Ok(v0)
      }
      AST::Call(name, args) => {
        if let Some(fun) = self.module.get_function(&name) {
          ...
          let v0 = self.builder_mut().build_call(fun, args);
          Ok(v0)
        } else {
      ...
      AST::Assign(..) | AST::Ident(..) => {
        ...
        // } else {
        //   let v0 = self.builder_mut().build_load(mem);
        //   Ok(v0)
        // }
      }
      _ => todo!(),
  ...
  fn gen_assign_impl(&mut self, mem: MemoryId, rhs: InstId) -> Expected<MemoryId> {
    ...
    // if true {
    //   self.builder_mut().build_store(mem, rhs);
    //   Ok(mem)
    // } else {
