...
fn irdump(f: &mut fmt::Formatter, module: &Module) -> fmt::Result {
  ...
  for (_id, fun) in module.functions().iter() {
    dump_fun(f, fun, module.functions())?;
  }
  Ok(())
}

fn dump_fun(f: &mut fmt::Formatter, fun: &Function, functions: &Arena<Function>) -> fmt::Result {
  ...
  for &block_id in fun.blocks() {
    ...
    for &inst_id in fun.get(block_id).insts() {
      dump_inst(f, &fun.get(inst_id), functions)?;
    }
  }
...
fn dump_inst(f: &mut fmt::Formatter, inst: &Inst, functions: &Arena<Function>) -> fmt::Result {
  match inst.kind() {
